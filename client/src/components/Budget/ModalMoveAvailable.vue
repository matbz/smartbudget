<template>
  <modal class="modal-popup" :name="name" :width="288" height="auto" :pivot-y="0.3" :pivot-x="0.45" @closed="closed" @opened="opened">
  <div class="modal-content">
    <div class="fieldset">
      <label>Move:</label>
      <div @keyup.esc="close()" @keyup.enter="save()">
        <vue-numeric
          class="modal-currency-input"
          currency="€"
    ...
    async save() {
      if (this.moveCategoryId) {
        ...
        if (moveCat.data.amount) {
          moveCatAmount = Number(moveCat.data.amount) + this.amount;
        } else {
          moveCatAmount = this.amount;
        }
